              ...
              <View style={styles.transactionLeft}>
                <View
                  style={[
                    ...
                    {
                      backgroundColor: transaction.type === 'payment' ? '#FF5F00' : '#2196F3',
                    },
                  ]}
                >
                  <MaterialIcons name={transaction.icon as any} size={20} color="#FFF" />
                </View>
                <View>
          ...
          <View style={styles.subscriptionsList}>
            {mySubscriptions.map((subscription) => (
              <View key={subscription.id} style={styles.subscriptionItem}>
                <Image source={{ uri: subscription.logo }} style={styles.subscriptionLogo} />
                <View style={styles.subscriptionInfo}>
                  <Text style={styles.subscriptionName}>{subscription.name}</Text>
                  <Text style={styles.subscriptionAmount}>{subscription.amount.toLocaleString()}원</Text>
                  <Text style={styles.subscriptionDate}>{subscription.dueDate} 결제 예정</Text>
                </View>
              </View>
            ))}
          </View>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.investmentScroll}>
            {investmentProducts.map((product) => (
              <TouchableOpacity key={product.id} style={styles.investmentCard}>
                <Image source={{ uri: product.image }} style={styles.investmentImage} />
                <View style={styles.investmentInfo}>
                  <Text style={styles.investmentTitle}>{product.title}</Text>
                  <Text style={styles.investmentDesc}>{product.description}</Text>
